...
def parseArguments():
    ...
    parser.add_argument("-psz", "--patch_size", 
                        ...
                        type=int, default=32)
    parser.add_argument("-edp", "--encoder_depths", 
                        help="Depth of encoder.", 
                        default=[3, 3, 9, 3])
    parser.add_argument("-edm", "--encoder_dims", 
                        help="Dimension of encoder.", 
                        default=[96, 192, 384, 768])
    parser.add_argument("-ddp", "--decoder_depth", 
                        help="Depth of decoder.", 
                        default=1)
    parser.add_argument("-ddm", "--decoder_dim", 
...
training_script1 = os.path.join(cur_dir, '../train_cnv2.py')
testing_script1 = os.path.join(cur_dir, '../test_cnv2.py')
training_script2 = os.path.join(cur_dir, '../train_cnv2_lp.py')
testing_script2 = os.path.join(cur_dir, '../test_cnv2_lp.py')
...
if user_input=='c':
    ...
    config['MAE TRAINING'] = {'use_prev_ae': args.use_prev_ae,
...
with open(script_fn, 'w') as f:
    ...
    for line in open(os.path.join(cur_dir,'minkowskiload.sh'), 'r').readlines():
        f.write(line)
    f.write('\n\n')
