...
def train_network(model, optimizer, lr_scheduler, cur_iter):
    ...
    while cur_iter < (total_batch_iters+15000):
        ...
        for target_train_batch in target_train_dataloader:
            ...
            (model, optimizer, 
             lr_scheduler, losses_cp) = run_iter(model, 
                                                 source_train_batch,
                                                 target_train_batch,
                                                 optimizer, 
                                                 lr_scheduler,
                                                 np.ones((len(source_mm_weights),)),
                                                 np.ones((len(source_um_weights),)),
                                                 0.0,
                                                 0.0,
                                                 np.zeros((len(source_task_weights),)),
                                                 np.zeros((len(target_task_weights),)),
                                                                 feat_loss_fn,
                                                                 losses_cp, 
                                                                 mode='predictor_train_mode')
            ...
            if cur_iter % verbose_iters == 0:
                ...
                print('\nBatch Iterations: %i/%i ' % (cur_iter, total_batch_iters+15000))
                print('Losses:')
            ...
            if cur_iter > (total_batch_iters+15000):
